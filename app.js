...
app.get('/index', async (req, res) => {
  try {
    ...
    res.render('index', { posts, sort: req.query.sort || 'desc' });
  } catch (error) {
...
app.post('/login', async (req, res) => {
    try {
        ...
        if (adminUser) {
            ...
            const redirectUrl = redirect || '/index';
            ...
            const redirectUrl = redirect || '/user-landing';
...
app.get('/admin/user-profiles', requireAdmin, async (req, res) => {
  try {
    ...
    // Render admin user profiles dashboard
...
app.get('/admin/user-profiles/:id', requireAdmin, async (req, res) => {
  try {
    ...
    if (userProfile.birthDate && userProfile.birthDate.month && userProfile.birthDate.year) {
      ...
      age = currentYear - userProfile.birthDate.year;
      
      if (userProfile.birthDate.month > currentMonth) {
        age--;
      }
    }
  ...
  try {
    ...
    const backups = await db.collection('database_backups')
      ...
      .then(backups => backups.map(backup => ({
        ...
        size: backup.size ? (backup.size / (1024 * 1024)).toFixed(2) + ' MB' : '0.05 MB', // Format size or provide default
        files: backup.files || [],
